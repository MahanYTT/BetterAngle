...
// HWND g_hHUD = NULL; // DEFINED IN State.cpp
...
LRESULT CALLBACK HUDWndProc(HWND hWnd, UINT msg, WPARAM wp, LPARAM lp) {
    ...
    case WM_MOUSEMOVE:
        if (g_isSelectionActive) {
            ...
            g_selectionRect = { min(g_startPoint.x, cur.x), min(g_startPoint.y, cur.y), max(g_startPoint.x, cur.x), max(g_startPoint.y, cur.y) };
    ...
    if (msg == WM_INPUT) {
        ...
        if (dwSize > 0) {
            ...
            if (GetRawInputData((HRAWINPUT)lp, RID_INPUT, lpb.data(), &dwSize, sizeof(RAWINPUTHEADER)) == dwSize) {
                ...
                if (raw->header.dwType == RIM_TYPEMOUSE) g_logic.ProcessInput(raw->data.mouse.lLastX, raw->data.mouse.lLastY);
            }
        }
    }
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE, LPWSTR, int) {
    ...
    // 3. DEFERRED SYSTEM INIT (v4.27.15)
