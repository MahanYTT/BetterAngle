...
LRESULT CALLBACK ThresholdWizProc(HWND hWnd, UINT message, WPARAM wParam,
                                  LPARAM lParam) {
  ...
  if (message == WM_INPUT) {
    if (!g_fortniteFocusedCache)
      return 0; // Prevent alt-tab and pre-focus tracking corruption
  ...
  if (message == WM_PAINT) {
    ...
    graphics.DrawString(L"MASTER CALIBRATION HUD", -1, &f,
                        Gdiplus::PointF(10, 10), &white);

    if (g_wizStep == 0) {
      graphics.DrawString(L"1. Look exactly center. Turn 120 degrees.", -1, &f2,
                          Gdiplus::PointF(10, 40), &amber);
      graphics.DrawString(L"2. Press F10.", -1, &f2, Gdiplus::PointF(10, 60),
                          &white);
    } else if (g_wizStep == 1) {
      graphics.DrawString(L"1. Jump out of the bus and FREEFALL.", -1, &f2,
                          Gdiplus::PointF(10, 40), &amber);
      graphics.DrawString(L"2. Turn exactly 120 degrees. Press F10.", -1, &f2,
                          Gdiplus::PointF(10, 60), &white);
    } else if (g_wizStep == 2) {
      graphics.DrawString(L"1. Deploy your GLIDER in the sky.", -1, &f2,
                          Gdiplus::PointF(10, 40), &amber);
      graphics.DrawString(L"2. Turn exactly 120 degrees. Press F10.", -1, &f2,
                          Gdiplus::PointF(10, 60), &white);
    }
...
void StartThresholdWizard(HINSTANCE hInstance) {
  ...
  g_hWiz =
      CreateWindowEx(WS_EX_TOPMOST | WS_EX_LAYERED | WS_EX_TOOLWINDOW,
                     L"ThresholdWizardWnd", L"Threshold Wizard", WS_POPUP,
                     sw / 2 - 175, 50, 350, 100, NULL, NULL, hInstance, NULL);
