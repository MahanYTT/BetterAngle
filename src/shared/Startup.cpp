...
void ShowSplashLoader(HINSTANCE hInst) {
  ...
  HWND hSplash =
      CreateWindowEx(WS_EX_TOPMOST | WS_EX_LAYERED, L"BetterAngleSplash",
                     L"Loading BetterAngle Pro", WS_POPUP, sx, sy, w, h, NULL,
                     NULL, hInst, NULL);
...

  {
    ...
    while (true) {
      ...
      if (current >= start) {
        ...
        // Handle wrap-around after 49.7 days
      ...
      g.DrawString(L"HIGH PERFORMANCE GAMING OPTIMIZER", -1, &gearFont,
                   PointF(42, 100), &neon);

      std::wstring status =
          progress < 0.7f ? L"INITIALIZING ENGINE..." : L"READY TO LOCK IN...";
      ...
      LinearGradientBrush grad(barRect, Color(255, 0, 200, 255),
                               Color(255, 0, 255, 200),
