...
void FindGameUserSettingsRecursive(const std::wstring &directory,
                                   std::vector<std::wstring> &outPaths) {
  if (outPaths.size() > 20)
    return; // Limit search to prevent hangs
...
double FetchFortniteSensitivity() {
  ...
  // Search from 'Saved' instead of just 'Config' to be MORE aggressive (covers
  // all edge cases)
  if (ExpandEnvironmentStringsW(L"%LOCALAPPDATA%\\FortniteGame\\Saved", expPath,
                                MAX_PATH) &&
      expPath[0] != L'%') {
    basePath = expPath;
  } else {
