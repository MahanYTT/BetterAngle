...
double FetchFortniteSensitivity() {
    ...
    std::wstring configPath = std::wstring(appdata) + L"\\FortniteGame\\Saved\\Config\\WindowsClient\\GameUserSettings.ini";
    
    std::error_code ec;
    if (!std::filesystem::exists(configPath, ec) || ec) {
        std::wcerr << L"FORTNITE SYNC: File not found at " << configPath << std::endl;
        return -1.0;
    }
    ...
    if (size <= 0 || size > 10 * 1024 * 1024) return -1.0;
