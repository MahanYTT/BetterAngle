...
void DrawOverlay(HWND hwnd, double angle, float detectionRatio, bool showCrosshair) {
    ...
    if (g_currentSelection != NONE) {
        ...
        if (g_currentSelection == SELECTING_ROI) {
            ...
            StretchBlt(hdcZoom, 0, 0, mw * 3, mh * 3, hdcScreen, mx, my, mw, mh, SRCCOPY);
    ...
    if (g_showROIBox && !g_allProfiles.empty()) {
        ...
        if (p.roi_w > 0 && p.roi_h > 0) {
            Color roiCol = g_isDiving ? Color(200, 255, 60, 60) : Color(200, 60, 220, 80);
            Pen roiPen(roiCol, 2.0f);
            REAL dash[] = { 8.0f, 4.0f };
            roiPen.SetDashPattern(dash, 2);
            graphics.DrawRectangle(&roiPen, p.roi_x - g_virtScreenX, p.roi_y - g_virtScreenY, p.roi_w, p.roi_h);
            FontFamily roiFF(L"Segoe UI");
            Font roiFont(&roiFF, 10, FontStyleBold, UnitPixel);
            SolidBrush roiLabel(roiCol);
            graphics.DrawString(g_isDiving ? L"DIVING" : L"GLIDING", -1, &roiFont, PointF(float(p.roi_x + 4 - g_virtScreenX), float(p.roi_y + 4 - g_virtScreenY)), &roiLabel);
        }
    }

    // Crosshair
    if (showCrosshair) {
        int primW = GetSystemMetrics(SM_CXSCREEN), primH = GetSystemMetrics(SM_CYSCREEN);
        ...
        float pulse = 1.0f;
        if (g_crossPulse) pulse = 0.6f + 0.4f * sinf(float(GetTickCount64()) * 0.005f);
        ...
        Matrix rot; rot.RotateAt(g_crossAngle, PointF(cx, cy));
    ...
    LinearGradientBrush bgBrush(Point(rx, ry), Point(rx, ry + rh), Color(150, 6, 8, 12), Color(150, 2, 3, 5));
    GraphicsPath path; AddRoundedRect(path, rx, ry, rw, rh, 8);
    graphics.FillPath(&bgBrush, &path);
    Color borderCol = g_isDiving ? Color(200, 255, 255, 255) : Color(90, 50, 65, 80);
    Pen borderPen(borderCol, 1.5f); graphics.DrawPath(&borderPen, &path);

    FontFamily ff(L"Segoe UI");
    Font labelFont(&ff, 9, FontStyleBold, UnitPixel);
    SolidBrush labelBrush(Color(160, 180, 185, 195));
    StringFormat fmtLabel; fmtLabel.SetAlignment(StringAlignmentCenter);
    graphics.DrawString(L"CURRENT ANGLE", -1, &labelFont, RectF(float(rx), float(ry + 8), float(rw), 18.0f), &fmtLabel, &labelBrush);
    ...
    StringFormat fmtAngle; fmtAngle.SetAlignment(StringAlignmentCenter); fmtAngle.SetLineAlignment(StringAlignmentNear);
    graphics.DrawString(angleStr.c_str(), -1, &angleFont, RectF(float(rx), float(ry + 28), float(rw), 80.0f), &fmtAngle, &angleBrush);
    ...
    graphics.DrawString(matchStr.c_str(), -1, &subFont, PointF(float(rx + 14), float(ry + rh - 54)), &matchLabelB);

    int barX = rx + 14, barY = ry + rh - 38, barW = rw - 28, barH = 8;
    SolidBrush barBgB(Color(60, 255, 255, 255)); graphics.FillRectangle(&barBgB, barX, barY, barW, barH);
    ...
    if (fillW > 0) {
        ...
        LinearGradientBrush barFill(Point(barX, barY), Point(barX + fillW, barY), Color(200, r, g, 40), Color(200, r / 2, g, 80));
        graphics.FillRectangle(&barFill, barX, barY, fillW, barH);
    }
    Pen barPen(Color(40, 255, 255, 255), 1.0f); graphics.DrawRectangle(&barPen, barX, barY, barW, barH);

    int swatchX = rx + rw - 28, swatchY = ry + 8;
    SolidBrush swatchB(Color(255, GetBValue(g_targetColor), GetGValue(g_targetColor), GetRValue(g_targetColor)));
    graphics.FillEllipse(&swatchB, swatchX, swatchY, 16, 16);
    Pen swatchP(Color(100, 220, 220, 220), 1.0f); graphics.DrawEllipse(&swatchP, swatchX, swatchY, 16, 16);
    ...
    graphics.DrawString(L":: drag", -1, &tinyFont, RectF(float(rx), float(ry + rh - 14), float(rw), 12.0f), &sfCenter, &tinyBrush);

    if (g_debugMode) {
        const int DBG_ROWS = 14, ROW_H = 18;
        int dw = 370, dh = 28 + DBG_ROWS * ROW_H + 10, dx = rx, dy = ry + rh + 10;
        if (dy + dh > sh) dy = ry - dh - 8;
        GraphicsPath dbgPath; AddRoundedRect(dbgPath, dx, dy, dw, dh, 6);
        LinearGradientBrush dbgBg(Point(dx, dy), Point(dx, dy + dh), Color(160, 8, 10, 14), Color(160, 3, 4, 6));
        graphics.FillPath(&dbgBg, &dbgPath);
        Pen dbgP(Color(80, 0, 190, 255), 1.0f); graphics.DrawPath(&dbgP, &dbgPath);
        SolidBrush headerBgB(Color(60, 0, 160, 255)); graphics.FillRectangle(&headerBgB, dx, dy, dw, 22);
        Font dbgTitle(&ff, 11, FontStyleBold, UnitPixel), dbgKey(&ff, 10, FontStyleBold, UnitPixel), dbgVal(&ff, 10, FontStyleRegular, UnitPixel);
        SolidBrush colTitle(Color(255, 255, 255, 255)), colKey(Color(255, 120, 180, 255)), colVal(Color(255, 220, 230, 240)), colGood(Color(255, 60, 230, 100)), colBad(Color(255, 255, 70, 70)), colWarn(Color(255, 255, 210, 50));
        graphics.DrawString(L"  DEBUG DASHBOARD", -1, &dbgTitle, PointF(float(dx + 4), float(dy + 5)), &colTitle);

        int row = 0;
        auto DrawRow = [&](const wchar_t* key, const std::wstring& val, SolidBrush* valBrush) {
            float rowY = float(dy + 28 + row * ROW_H);
            graphics.DrawString(key, -1, &dbgKey, PointF(float(dx + 8), rowY), &colKey);
            graphics.DrawString(val.c_str(), -1, &dbgVal, PointF(float(dx + 175), rowY), valBrush);
            if (row > 0) { Pen lnP(Color(20, 255, 255, 255), 1.0f); graphics.DrawLine(&lnP, dx + 4, int(rowY) - 1, dx + dw - 4, int(rowY) - 1); }
            row++;
        };
        ...
        DrawRow(L"Detection Ratio", FmtFloat(detectionRatio * 100.0, 1) + L"% / match " + std::to_wstring(matchPct) + L"%", matchPct > 5 ? &colGood : &colVal);
        ...
        std::wstring roiStr = !g_allProfiles.empty() ? (L"x" + std::to_wstring(g_allProfiles[g_selectedProfileIdx].roi_x) + L" y" + std::to_wstring(g_allProfiles[g_selectedProfileIdx].roi_y) + L" " + std::to_wstring(g_allProfiles[g_selectedProfileIdx].roi_w) + L"x" + std::to_wstring(g_allProfiles[g_selectedProfileIdx].roi_h)) : L"-";
